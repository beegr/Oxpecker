...
let ``subRoute: configureEndpoint inside subRoute`` () =
    task {
        ...
        let endpoints = [
            route "/" (fun ctx ->
                rootMetadata <- ctx.GetEndpoint() |> Option.ofObj |> Option.get |> _.Metadata
                ctx.WriteText "")
            GET [
                route "/get" (fun ctx ->
                    getMetadata <- ctx.GetEndpoint() |> Option.ofObj |> Option.get |> _.Metadata
                    ctx.WriteText "Hello World")
                subRoute "/api" [
                    routef "/inner" (fun ctx ->
                        innerMetadata <- ctx.GetEndpoint() |> Option.ofObj |> Option.get |> _.Metadata
                        ctx.WriteText "Hi")
                ]
